...
We will be designing our device so that it takes full advantage of the method's strong points. It will therefore make use of the scalability of this technology and be designed to be as wearable, discrete and compact as possible. The goal being to obtain a device that is practical in daily use over long periods of time without restraining the user as this seems to be one of the main obstacles haptic feedback is facing.
	%\subsection{On-board vs delegated processing}
	%Whether it is raw data coming from sensors or high level information coming from a phone there has to be some processing to turn the data into an adequate signal sent to the electromagnets. This can either be done by the device itself (on-board) or we can suppose that it has already been done at the source and that the device is the equivalent of a passive speaker (delegated).
	%The first option has the advantage of being easily compatible without any required software on the machine the device is connected to. On the other hand it makes the device more complex since a microprocessor has to be included and programmed. 
	%The alternative is much simpler as the device would only consist of amplifiers and electromagnets but it also implies that it has to be addressed to through dedicated software.
	...
	%\subsection{Setup an initial prototype and iteratively improve on it}
	%Once an optimal setup is found we can construct an initial prototype and by trial and %error improve it's efficiency and practicality until we have a device that we can %reliably test with.
